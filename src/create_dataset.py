    ...
    def split_data(
        ...
        for start_idx in range(state_set.shape[1] - self.max_length + 1):
            ...
            if next_state_set is not None:
                next_state_piece = next_state_set[
                    :, start_idx : start_idx + self.max_length, :
                ]  # (k, 20, 4)
            ...
            action_piece = action_set[
                :, start_idx : start_idx + self.max_length, :
            ]  # (k, 20, 4)
            a.append(action_piece)
            timesteps = np.arange(self.max_length)
            expand_timesteps = np.tile(timesteps, state_set.shape[0]).reshape(
                state_set.shape[0], timesteps.shape[0]
            )
        ...
        norm_s = torch.from_numpy(np.concatenate(norm_s, axis=0)).to(
            dtype=torch.float32, device=self.device
        )
        ...
        t = torch.from_numpy(np.concatenate(t, axis=0)).to(
            dtype=torch.float32, device=self.device
        )
        rtg = torch.from_numpy(np.concatenate(rt, axis=0)).to(
            dtype=torch.float32, device=self.device
        )
