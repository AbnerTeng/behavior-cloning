...
class TradeEnv(gym.Env):
    ...
    def define_state_space(self) -> spaces.Space:
        """
    ...
    def step(self, action: int) -> Tuple[torch.Tensor, float, bool, dict]:
        ...
        """
        if self.denorm_data is not None:
            self.denorm_data = self.denorm_data.cpu()
            close_today = self.denorm_data[
                self.current_idx, 3
            ]  # 3 consists of the close price
            close_next = self.denorm_data[self.current_idx + 1, 3]
            ...
            raise ValueError("Denormalized data is required")
    ...
    def calculate_reward(self) -> float:
        if self.denorm_data:
            new_portfolio_value = self.balance + (
                self.shares_held * self.denorm_data[self.current_idx + 1, 3, -1]
            )
            reward = new_portfolio_value - (
                self.balance
                + (self.shares_held * self.denorm_data[self.current_idx, 3, -1])
            )
        else:
            reward = 0.0
